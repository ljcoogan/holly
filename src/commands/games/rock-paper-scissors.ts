...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const provided = answers.findIndex((answer) => message === answer);

    if (provided === -1) interaction.reply("Hey! This isn't a valid move!");
    else if (response === 0)
      interaction.reply(`I played ${answers[provided]}. We draw.`);
    else if (response === 1)
      interaction.reply(`I played ${answers[(provided + 1) % 3]}. You lose!`);
    else if (response === 2)
      interaction.reply(`I played ${answers[(provided + 2) % 3]}. You win!`);
  },
};
