...
8. That's it! Run `npm run dev` to start Holly! The bot has started when you see "Ready! Logged in as `x`", where `x` is your bot's Discord account. If you want the bot to run constantly in the background, consider using the provided `holly.service`.
